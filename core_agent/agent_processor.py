...
from bot_loader import dp, bot as bot_instance # Импортируем бот для get_me и передачи в Pro
...
async def _execute_pro_model_logic(
    ...
    """Выполняет стандартную логику обработки Pro моделью."""
    ...
    logger_ap.debug(f"Executing Pro model logic for chat {chat_id}")

    try:
        # --- Подготовка истории ---
        initial_history_dict, original_db_len = await prepare_history(
            chat_id=chat_id,
            user_id=user_id,
            chat_type=chat_type,
        ...
        final_history_obj_list, interaction_error_msg, last_func_name, last_sent_text, last_func_result = await process_request(
            ...
            initial_history=initial_history_dict,
        ...
        # --- Обработка результата Pro AI ---
        if interaction_error_msg:
            logger_ap.error(f"Core Agent (Pro): AI interaction failed for chat {chat_id}: {interaction_error_msg}")
            error_message_for_user = f"Произошла ошибка при обработке: {interaction_error_msg}"
        elif final_history_obj_list:
            # Сначала извлекаем текст, который модель могла сгенерировать
            final_response_text_raw = extract_text(final_history_obj_list)

            # --- ПРОВЕРКА ДЛЯ ПОДАВЛЕНИЯ ДУБЛИРОВАНИЯ ---
            # Если последним действием был send_telegram_message И модель все равно что-то написала текстом,
            # то мы этот текст игнорируем, т.к. сообщение уже было отправлено инструментом.
            if last_func_name == 'send_telegram_message' and final_response_text_raw:
                ...
                final_response_text_raw = None # Обнуляем текст, чтобы он не отправлялся
            # --- КОНЕЦ ПРОВЕРКИ ---

            # Теперь обрабатываем текст (если он остался)
            if final_response_text_raw:
                ...
                # Логируем, если текста нет (либо модель не сгенерировала, либо мы подавили)
                reason = "Model generated no text" if last_func_name != 'send_telegram_message' else "Text suppressed after send_telegram_message"
                log_level = logging.INFO if last_func_name else logging.WARNING # Если и функции не было, то WARNING
                logger_ap.log(log_level, f"Core Agent (Pro): No final text to send for chat {chat_id}. Reason: {reason}. (Last func: {last_func_name})")


            # --- Сохранение истории Pro ---
            # Сохраняем ВСЮ историю, включая финальный ответ модели (даже если мы его подавили для пользователя)
            # И включая вызов send_telegram_message
            if save_history: # Проверяем импорт save_history
                await save_history(
                      chat_id=chat_id,
                      final_history_obj_list=final_history_obj_list,
                      original_db_history_len=original_db_len,
                      current_user_id=user_id,
                      # Передаем текст ПОСЛЕДНЕГО УСПЕШНОГО вызова send_telegram_message (если он был)
                      last_sent_message_text=last_sent_text
                )
            else:
                 logger_ap.error(f"Cannot save Pro history for chat {chat_id}: save_history function is not available.")
        else:
            # Случай, когда process_request вернул пустую историю без ошибки
            logger_ap.error(f"Core Agent (Pro): AI interaction returned None history without error msg for chat {chat_id}")
            error_message_for_user = "Модель AI не вернула корректный результат."

        # --- Возврат результата Pro ---
        if error_message_for_user:
            # Возвращаем текст ошибки (уже должен быть экранирован, если пришел изнутри)
            # На всякий случай экранируем еще раз
            return escape_markdown_v2(error_message_for_user)
        elif final_response_text_escaped:
            # Возвращаем финальный текст (если он не был подавлен)
            return final_response_text_escaped
        else:
            # Если текста нет (подавлен или не сгенерирован)
            return None
...
async def handle_user_request(
    ...
    В ЛС всегда вызывает Pro.
    В группах: вызывает Pro при ответе/упоминании, иначе вызывает Lite-фильтр.
    """
    chat_id = message.chat.id
    ...
    # --- Сохранение сообщения и профиля пользователя СРАЗУ ---
    if user and user_input: # Убедимся, что user не None
        try:
            ...
            try:
                ...
                # Проверяем доступность database перед вызовом
    ...
    lite_model = dp.workflow_data.get("lite_model")
    pro_model = dp.workflow_data.get("pro_model")
    available_pro_functions = dp.workflow_data.get("available_pro_functions", {})
    max_pro_steps = dp.workflow_data.get("max_pro_steps", 10)

    # Проверка наличия Pro модели (критично)
    if not pro_model:
        logger_ap.critical(f"Core Agent: Pro model not found in workflow_data for chat {chat_id}")
        return escape_markdown_v2("⚠️ Ошибка: Основная модель AI недоступна.")
    if not bot_instance: # Проверка инстанса бота
    ...
    if force_pro_model:
        ...
        # Проверка ответа/упоминания в группе
        ...
        if bot_info and bot_info.id:
            ...
            if BOT_INFO_CACHE["username_lower"]:
                # Улучшенный паттерн для проверки упоминания (включая начало строки)
        ...
        if is_reply_to_bot or is_mention:
            ...
            # Не реплай/упоминание -> возможно, Lite-фильтр
            if lite_model:
                call_lite_filter = True
                logger_ap.info(f"Lite filter will be used for group message (user {user_id} chat {chat_id}).")
            else:
                call_pro_directly = True
                pro_reason = "Lite filter unavailable for group"
                logger_ap.warning(f"Lite model unavailable for group chat {chat_id}, falling back to Pro.")
    else:
        call_pro_directly = True
    ...
    if call_lite_filter:
        try:
            ...
            # Используем настройки генерации и безопасности для Lite, если они заданы в модели
            lite_response = await lite_model.generate_content_async(lite_input)
            ...
            if isinstance(parse_result, str) and parse_result == "NO_ACTION_NEEDED":
                ...
                return None # Завершаем, ничего не делаем
                ...
                if any(action.get("function_name") == "trigger_pro_model_processing" for action in actions_from_lite):
                    ...
                    pro_reason = "Lite filter requested Pro" # Обновляем причину
                ...
                call_pro_directly = True # Fallback
            else:
    ...
    # --- 2. Выполнение действий из Lite (remember_user_info) ---
    # Выполняем, только если Lite отработал успешно и вернул список действий
    remember_only_action = False
    if actions_from_lite is not None: # Проверяем, что actions_from_lite - это список
        remember_action_found = False
        pro_action_found = False # Проверяем, было ли действие вызова Pro
        ...
        for action in actions_from_lite:
            ...
            if func_name == "remember_user_info":
                ...
                if database and hasattr(database, 'upsert_user_note') and 'user_id' in args and 'info_category' in args and 'info_value' in args:
                    try:
                        # Передаем аргументы напрямую, т.к. парсер их уже подготовил
                        # Убедимся, что вызываем правильную функцию БД
            ...
            # Здесь можно добавить обработку других потенциальных быстрых действий от Lite

        # Если единственным(и) действием(ями) было запоминание И Lite не просил вызвать Pro
        if remember_action_found and not pro_action_found and not trigger_pro_after_lite:
            ...
            return None # Ответ пользователю не нужен

    # --- 3. Вызов Pro-модели (если нужно) ---
    if call_pro_directly or trigger_pro_after_lite:
        # Логируем только если действительно вызываем Pro
        if not pro_reason: pro_reason = "Fallback or Unknown" # На случай, если причина не установилась
        logger_ap.info(f"Proceeding with Pro model for user {user_id} chat {chat_id} (Reason: {pro_reason}).")
        try:
            # Вызываем _execute_pro_model_logic, которая вернет текст или None
            return await _execute_pro_model_logic(
             ...
             # Возвращаем сообщение об ошибке для отправки пользователю
    ...
    # Сюда можно попасть, если Lite вернул пустой список действий (не "NO_ACTION_NEEDED")
    # или какие-то другие действия, которые не требуют вызова Pro и не были "remember".
